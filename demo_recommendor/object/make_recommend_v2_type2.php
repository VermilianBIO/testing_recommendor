...
foreach($tmp as $k){
	...
	foreach($k as $element){
		...
		foreach($element as $key=>$value){
			//echo $key;
			if(isset($priority_table[$key])){
				$priority_table[$key]++;
			}else{
				$priority_table[$key] = 1;
			}
		}
	}
}
arsort($priority_table);
//print_r($priority_table);
...
foreach(array_unique($summed_score) as $unique){
	...
	foreach($key_of_unique as $key){
		...
		if(array_key_exists($key,$sold_count_array)){
			...
			$summed_score[$key] = array("priority"=>$priority_table[$key],"score"=>$summed_score[$key],"sold"=>$sold_count_array[$key],"name"=>$score_array[$key][0]["GroupName"],"image"=>$score_array[$key][0]["GroupImage"]);
		}
	}
...
$summed_score = count($summed_score)>3? array_slice($summed_score,0,3,true) : $summed_score;
